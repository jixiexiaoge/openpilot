...
static uint32_t changan_get_checksum(const CANPacket_t *to_push) {
  ...
  if (addr == 0x307 || addr == 0x31A) {
    return (uint32_t)GET_BYTE(to_push, 7); // Simplified, usually individual blocks are checked by OP
  }
...
static void changan_rx_hook(const CANPacket_t *to_push) {
  if (GET_BUS(to_push) == CHANGAN_MAIN) {
    ...
    if (addr == CHANGAN_STEER_TORQUE) {
      // Signal: EPS_MeasuredTorsionBarTorque (7|16@0-, factor 0.001)
    ...
    if (addr == CHANGAN_CRUISE_BUTTONS) {
      // Signal: GW_MFS_IACCenable_switch_signal at bit 0
      // Signal: GW_MFS_Cancle_switch_signal at bit 1
    ...
    if (addr == CHANGAN_PEDAL_DATA) {
      // Signal: EMS_BrakePedalStatus at 54 (Byte 6), EMS_RealAccPedal at 20 (Byte 2)
...
static bool changan_tx_hook(const CANPacket_t *to_send) {
  static const AngleSteeringLimits CHANGAN_STEERING_LIMITS = {
    .max_angle = 1300,       // 130.0 deg (aligned with referencing max_steering_angle)
    .angle_deg_to_can = 10,
    .angle_rate_up_lookup = {{0., 100.}, {5., 5.}}, // Relaxed
    .angle_rate_down_lookup = {{0., 100.}, {5., 5.}},
  };
  ...
  if (addr == CHANGAN_STEER_COMMAND && bus == CHANGAN_MAIN) {
...
static int changan_fwd_hook(int bus, int addr) {
  ...
  if (bus == CHANGAN_MAIN) {
    ...
    bool block = (addr == CHANGAN_STEER_COMMAND) || (addr == CHANGAN_ACC_COMMAND) ||
                 (addr == CHANGAN_ACC_HUD) || (addr == CHANGAN_ADAS_INFO);
    if (!block) {
...
static safety_config changan_init(uint16_t param) {
  static const CanMsg CHANGAN_TX_MSGS[] = {
    ...
    {CHANGAN_STEER_TORQUE,  CHANGAN_CAM,  8}, // Sent to Bus 2 as per reference code
  };

  static RxCheck changan_rx_checks[] = {
    {.msg = {{CHANGAN_STEER_ANGLE,    CHANGAN_MAIN, 8, .frequency = 100U}, { 0 }, { 0 }}},
    {.msg = {{CHANGAN_STEER_TORQUE,   CHANGAN_MAIN, 8, .frequency = 100U}, { 0 }, { 0 }}},
    {.msg = {{CHANGAN_CRUISE_BUTTONS, CHANGAN_MAIN, 8, .frequency = 50U}, { 0 }, { 0 }}},
    {.msg = {{CHANGAN_WHEEL_SPEEDS,   CHANGAN_MAIN, 8, .frequency = 100U}, { 0 }, { 0 }}},
  };
