...
#define TESLA_COMMON_RX_CHECKS \
  ...
  {.msg = {{0x145, 0, 8, .max_counter = 15U, .quality_flag = true, .frequency = 50U}, { 0 }, { 0 }}},   /* ESP_status (brakes) */                          \
...
#define TESLA_VEHICLE_BUS_ADDR_CHECK \
  {.msg = {{0x3DF, 1, 8, .ignore_checksum = true, .ignore_counter = true, .frequency = 2U}, { 0 }, { 0 }}},  /* UI_status2 */ \

static bool tesla_longitudinal = false;
static bool tesla_stock_aeb = false;

// Stock steering control passthrough (LDA, ELDA, Autopark interventions on bus 2)
// Only rising edges while controls are not allowed are considered
static bool tesla_stock_steering_control = false;
static bool tesla_stock_steering_control_prev = false;

// Detected VEHICLE bus harness (enables DAS_bodyControls blinker MITM on bus 1)
static bool tesla_has_vehicle_bus = false;
...
static bool tesla_get_quality_flag_valid(const CANPacket_t *to_push) {
  ...
  if (addr == 0x155) {
    ...
    int user_brake_status = (GET_BYTE(to_push, 3) >> 5) & 0x03U;
...
static void tesla_rx_hook(const CANPacket_t *to_push) {
  ...
  if (bus == 0) {
    ...
    if (addr == 0x155) {
      ...
      const float TESLA_MAX_SPEED_DELTA = 2.0;  // m/s
      ...
      vehicle_moving = !GET_BIT(to_push, 41U);  // ESP_vehicleStandstillSts
    }
  ...
  if (bus == 2) {
    // DAS_control: monitor stock AEB
    if (addr == 0x2b9) {
      // "AEB_ACTIVE"
      tesla_stock_aeb = (GET_BYTE(to_push, 2) & 0x03U) == 1U;
    }

    // DAS_steeringControl: monitor stock steering control (LDA, ELDA, Autopark)
    if (addr == 0x488) {
...
static bool tesla_tx_hook(const CANPacket_t *to_send) {
  ...
  // bits, so we don't value-check it here. The TX whitelist still gates the address/bus.
...
static int tesla_fwd_hook(CANPacket_t* to_send) {
  ...
  if (bus_num == 2) {
    ...
    // DAS_steeringControl: forwarded while stock steering control is active (LDA, ELDA, Autopark)
...
static safety_config tesla_init(uint16_t param) {
  ...
  // With vehicle bus harness: adds DAS_bodyControls on bus 1 for nav blinker control
  static const CanMsg TESLA_M3_Y_VEHICLE_BUS_TX_MSGS[] = {
    {0x488, 0, 4},  // DAS_steeringControl
    {0x2b9, 0, 8},  // DAS_control
    {0x27D, 0, 3},  // APS_eacMonitor
    {0x3E9, 1, 8},  // DAS_bodyControls (blinker)
  };
  ...
  const uint16_t TESLA_FLAG_VEHICLE_BUS = 4;
  tesla_has_vehicle_bus = GET_FLAG(param, TESLA_FLAG_VEHICLE_BUS);
  ...
  if (tesla_has_vehicle_bus) {
    SET_TX_MSGS(TESLA_M3_Y_VEHICLE_BUS_TX_MSGS, ret);
