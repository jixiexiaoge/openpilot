...
class AngleSteeringLimits:
  ...
  # v2 — VehicleModel-based limits (optional, defaults = inactive)
  MAX_LATERAL_ACCEL: float = 0       # m/s^2, 0 = unused
  MAX_LATERAL_JERK: float = 0        # m/s^3, 0 = unused
  MAX_ANGLE_RATE: float = math.inf   # deg/frame, inf = unused
...
def get_max_angle_delta_vm(v_ego_raw: float, VM, limits):
...
def get_max_angle_vm(v_ego_raw: float, VM, limits):
