...
class LongControlJerk():
  ...
  def update(self, enabled, override, distance, has_lead, accel, critical_state):
    if not enabled:
      ...
      jerk_limit_min_target = self.JERK_LIMIT_MIN if has_lead else self.JERK_LIMIT_MIN_NO_LEAD
      ...
      if has_lead:
        ...
        filter_gain_dist = np.interp(distance, self.FILTER_GAIN_DISTANCE, [self.FILTER_GAIN_MAX, self.jerk_limit_min])
        filter_gain_dist_change = np.interp(abs(distance_change), self.FILTER_GAIN_DISTANCE_CHANGE, [self.jerk_limit_min, self.FILTER_GAIN_MAX])
        filter_gain = max(filter_gain_dist, filter_gain_dist_change)
      else:
...
class LongControlLimit():
  ...
  LIMIT_DISTANCE_CHANGE_DOWN = [0, 20]
  LIMIT_DISTANCE_CHANGE_UP = [0, 5]
  LIMIT_DISTANCE_CHANGE_UP_ACT = [0, 60]
  DISTANCE_FILTER_RC = [0.15, 0.6]
  DISTANCE_TIMEOUT = 1.
  ...
  def update(self, enabled: bool, speed: float, set_speed: float, distance: float, has_lead: bool, critical_state: bool):
    if not enabled or critical_state:
      self.upper_limit = self.LIMIT_MIN
      self.lower_limit = self.LIMIT_MIN
      self.distance_valid_timer = 0
    elif not has_lead:
      if self.distance_valid_timer < self.DISTANCE_TIMEOUT:
      ...
      if (self.distance_last == 0 or self.distance_valid_timer != 0) and distance != 0:
      ...
      upper_limit_dist = np.interp(distance, self.LIMIT_DISTANCE, [self.LIMIT_MIN, self.UPPER_LIMIT_MAX])
      upper_limit_dist_change = np.interp(-min(0, distance_change), self.LIMIT_DISTANCE_CHANGE_UP, [self.UPPER_LIMIT_MAX, self.LIMIT_MIN])
      upper_limit_dist_change = np.interp(distance, self.LIMIT_DISTANCE_CHANGE_UP_ACT, [upper_limit_dist_change, upper_limit_dist])
      self.upper_limit = min(upper_limit_dist, upper_limit_dist_change)

      set_speed_diff_up = max(0, abs(speed) - abs(set_speed))
      set_speed_diff_up_factor = np.interp(set_speed_diff_up, [1, 1.75], [1., 0.])
      lower_limit_dist = np.interp(distance, self.LIMIT_DISTANCE, [self.LOWER_LIMIT_MIN, self.LOWER_LIMIT_MAX])
      lower_limit_dist_speed = lower_limit_dist * set_speed_diff_up_factor
      lower_limit_dist_change = np.interp(max(0, distance_change), self.LIMIT_DISTANCE_CHANGE_DOWN, [self.LOWER_LIMIT_MAX, self.LIMIT_MIN])
      self.lower_limit = min(lower_limit_dist_speed, lower_limit_dist_change)
...
def sigmoid_curvature_boost_meb(kappa: float, v_ego: float, kappa_thresh: float = 0.0) -> float:
  ...
  boost_values = np.array([1.5, 2.1])
  boost = float(np.interp(v_ego, v_points, boost_values))
  steepness_values = np.array([5000.0, 3200.0])
...
def get_acc_warning_meb(self, acc_hud):
  if (acc_hud["ACC_Status_ACC"] in (3, 4)
      and acc_hud["ACC_EGO_Fahrzeug"] == 2
      and acc_hud["ACC_Optischer_Fahrerhinweis"] != 0
      and acc_hud["ACC_Akustischer_Fahrerhinweis"] != 0
      and acc_hud["ACC_Display_Prio"] == 0):
    return True
  return False
...
class LatControlCurvature():
  ...
  def update(self, CS, CC, desired_curvature):
    actual_curvature_vm = CC.currentCurvature
