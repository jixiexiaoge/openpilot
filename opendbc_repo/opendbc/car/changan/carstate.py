...
class CarState(CarStateBase):
  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    iacc_button = self.cruise_buttons["GW_MFS_ACC"]
    ...
    if self.cruiseEnable:
      if self.cruise_buttons["GW_MFS_RESPlus"] == 1 and self.buttonPlus == 0:
        self.cruiseSpeed = ((self.cruiseSpeed // 5) + 1) * 5
      if self.cruise_buttons["GW_MFS_SETReduce"] == 1 and self.buttonReduce == 0:
        self.cruiseSpeed = max(((self.cruiseSpeed // 5) - 1) * 5, 0)

    self.buttonPlus = self.cruise_buttons["GW_MFS_RESPlus"]
    self.buttonReduce = self.cruise_buttons["GW_MFS_SETReduce"]
