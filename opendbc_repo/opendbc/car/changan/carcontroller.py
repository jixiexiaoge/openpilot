...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if CC.latActive:
      ...
      can_sends.append(changancan.create_steering_control(self.packer, CS.sigs["STEER_COMMAND"], apply_angle, 1, (self.frame // 1) % 16))
    else:
      apply_angle = CS.out.steeringAngleDeg
      self.filtered_steering_angle = apply_angle
      can_sends.append(changancan.create_steering_control(self.packer, CS.sigs["STEER_COMMAND"], apply_angle, 0, (self.frame // 1) % 16))
    ...
    if CC.longActive:
      ...
      can_sends.append(changancan.create_acc_control(self.packer, CS.sigs["ACC_COMMAND"], accel, (self.frame // 1) % 16, True, acctrq))
    ...
    if self.frame % 10 == 0:
      ...
      can_sends.append(changancan.create_acc_hud(self.packer, CS.sigs["ADAS_INFO"], (self.frame // 10) % 16, CC.longActive, CS.out.steeringPressed))

    # EPS Control (100Hz)
    can_sends.append(changancan.create_eps_control(self.packer, CS.sigs["STEER_TORQUE"], CC.latActive, self.frame % 16))
