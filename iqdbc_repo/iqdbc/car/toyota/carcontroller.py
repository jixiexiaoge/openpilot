import math
import numpy as np
from iqdbc.car import Bus, make_tester_present_msg, rate_limit, structs, ACCELERATION_DUE_TO_GRAVITY, DT_CTRL
from iqdbc.car.lateral import apply_meas_steer_torque_limits, apply_std_steer_angle_limits, common_fault_avoidance
from iqdbc.car.carlog import carlog
from iqdbc.car.common.filter_simple import FirstOrderFilter, HighPassFilter
from iqdbc.car.common.pid import PIDController
from iqdbc.car.secoc import add_mac, build_sync_mac
from iqdbc.car.interfaces import CarControllerBase
from iqdbc.car.toyota import toyotacan
from iqdbc.car.toyota.values import CAR, NO_STOP_TIMER_CAR, TSS2_CAR, \
...
class CarController(CarControllerBase, GasInterceptorCarController):
  ...
  def update(self, CC, CC_IQ, CS, now_nanos):
    ...
    # SNG hack: skip setting standstill_req so RELEASE_STANDSTILL stays True and openpilot controls through the stop
    if self.CP.carFingerprint not in NO_STOP_TIMER_CAR or self.CP_IQ.enableGasInterceptor:
      if CS.out.standstill and not self.last_standstill and \
         (self.CP_IQ.enableGasInterceptor or not (self.CP_IQ.flags & ToyotaFlagsIQ.STOP_AND_GO_HACK)):
      ...
      # NO_STOP_TIMER cars: actively manage standstill hold/release based on planner intent.
