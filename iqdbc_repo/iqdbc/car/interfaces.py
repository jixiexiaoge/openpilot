...
from iqdbc.car import DT_CTRL, apply_hysteresis, gen_empty_fingerprint, scale_rot_inertia, scale_tire_stiffness, STD_CARGO_KG
...
from iqdbc.car.common.simple_kalman import KF1D, get_kalman_gain
...
class CarInterfaceBase(ABC, CarInterfaceBaseIQ):
  ...
  def __init__(self, CP: structs.CarParams, CP_IQ: structs.IQCarParams):
    ...
    self.CS: CarStateBase = self.CarState(CP, CP_IQ)
    self.can_parsers: dict[StrEnum, CANParser] = self.CS.get_can_parsers(CP, CP_IQ)
  ...
  @classmethod
  def get_non_essential_params_iq(cls, car_params, candidate: str) -> structs.IQCarParams:
    return cls.get_params_iq(car_params, candidate, gen_empty_fingerprint(), list(), False, False, False)
  ...
  @classmethod
  def get_params_iq(cls, car_params, candidate: str, fingerprint: dict[int, dict[int, int]], car_fw: list[structs.CarParams.CarFw], alpha_long: bool,
                    is_release_iq: bool, docs: bool) -> structs.IQCarParams:
    car_params_iq = structs.IQCarParams()

    platform = PLATFORMS[candidate]
    car_params_iq.flags |= int(platform.config.iq_flags)
    car_params_iq.pcmCruiseSpeed = True

    return cls._get_params_iq(car_params, car_params_iq, candidate, fingerprint, car_fw, alpha_long, is_release_iq, docs)
  ...
  def _get_params_iq(stock_cp: structs.CarParams, ret: structs.IQCarParams, candidate, fingerprint: dict[int, dict[int, int]],
    ...
    carlog.debug(f"Car {candidate} does not have a _get_params_iq method, using defaults")
    return ret

  @classmethod
  def get_longitudinal_tuning_iq(cls, stock_cp: structs.CarParams, ret: structs.IQCarParams) -> structs.IQCarParams:
    return cls._get_longitudinal_tuning_iq(stock_cp, ret)
  ...
  def _get_longitudinal_tuning_iq(stock_cp: structs.CarParams, ret: structs.IQCarParams) -> structs.IQCarParams:
    ...
    carlog.debug(f"Car {stock_cp.carFingerprint} does not have a _get_longitudinal_tuning_iq method, using defaults")
    return ret

  @staticmethod
  def pre_init(CP: structs.CarParams, CP_IQ: structs.IQCarParams, can_recv: CanRecvCallable, can_send: CanSendCallable):
    """Used to check conditions to disable longitudinal ECUs as needed and set/change car params"""

  @staticmethod
  def init(CP: structs.CarParams, CP_IQ: structs.IQCarParams, can_recv: CanRecvCallable, can_send: CanSendCallable):
    """Used to disable longitudinal ECUs as needed"""

  @staticmethod
  def pre_init(CP: structs.CarParams, CP_IQ: structs.IQCarParams, can_recv: CanRecvCallable, can_send: CanSendCallable):
    """Optional early setup hook used before interface initialization."""
  ...
  def update(self, can_packets: list[tuple[int, list[CanData]]]) -> tuple[structs.CarState, structs.IQCarState]:
    # parse can
    ...
    # get CarState
    ret, ret_iq = self.CS.update(self.can_parsers)
...
class CarStateBase(ABC):
  def __init__(self, CP: structs.CarParams, CP_IQ: structs.IQCarParams):
    ...
    self.out_iq = structs.IQCarState()
  ...
  @staticmethod
  def get_can_parsers(CP, CP_IQ) -> dict[StrEnum, CANParser]:
    return {}


class CarControllerBase(ABC):
  def __init__(self, dbc_names: dict[StrEnum, str], CP: structs.CarParams, CP_IQ: structs.IQCarParams):
    self.CP = CP
    self.CP_IQ = CP_IQ
    self.frame = 0
    self.secoc_key: bytes = b"00" * 16
...
def get_interface_attr(attr: str, combine_brands: bool = False, ignore_none: bool = False) -> dict[str | StrEnum, Any]:
  # read all the folders in iqdbc/car and return a dict where:
  ...
  for car_folder in sorted([x[0] for x in os.walk(BASEDIR)]):
    try:
      ...
      brand_values = __import__(f'iqdbc.car.{brand_name}.{INTERFACE_ATTR_FILE.get(attr, "values")}', fromlist=[attr])
