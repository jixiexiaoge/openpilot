...
class SpeedLimitController:
  ...
  def update_gps(self, sm):
    ...
    elif llk_valid and llk is not None and getattr(llk, "positionGeodetic", None) and llk.positionGeodetic.valid:
      self.gps_valid = True
      self.gps_position = {
        "bearing": math.degrees(llk.calibratedOrientationNED.value[2]) if getattr(llk, "calibratedOrientationNED", None) else 0,
  ...
  def update_limits(self, dashboard_speed_limit, now, time_validated, v_cruise, v_ego, sm, slc_params):
    ...
    use_mapbox = slc_params.get("slc_mapbox_filler", False)
    if use_mapbox:
      self._maybe_reset_mapbox_quota(now, time_validated)
    if use_mapbox and self.mapbox_requests["total_requests"] < self.mapbox_requests["max_requests"]:
    ...
    resolved_limit, resolved_source = self._resolver.resolve(dashboard_limit, self.mapbox_limit, slc_params)
