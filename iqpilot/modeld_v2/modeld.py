...
from openpilot.iqpilot.modeld_v2.fill_model_msg import fill_model_msg, fill_pose_msg, PublishState, get_curvature_from_output
from openpilot.iqpilot.modeld_v2.constants import Plan
from openpilot.iqpilot.modeld_v2.models.commonmodel_pyx import DrivingModelFrame, CLContext
from openpilot.iqpilot.modeld_v2.meta_helper import load_meta_constants
from openpilot.iqpilot.modeld_v2.camera_offset_helper import CameraOffsetHelper
...
from openpilot.iqpilot.models.runners.helpers import get_model_runner
...
class ModelState(ModelStateBase):
  ...
  def run(self, bufs: dict[str, VisionBuf], transforms: dict[str, np.ndarray],
    ...
    # fused runner warps and manages temporal buffers inside the JIT; feed it raw NV12
    if not self.model_runner.uses_opencl_warp:
      if prepare_only:
        return None
      return self.model_runner.run_fused(bufs, transforms, inputs)
  ...
  def get_action_from_model(self, model_output: dict[str, np.ndarray], prev_action: log.ModelDataV2.Action,
    ...
    if IQP_NAV_MODEL_INFLUENCE_ENABLED and 'planplus' in model_output and self.PLANPLUS_CONTROL != 1.0:
...
def main(demo=False):
  ...
  # messaging
  pm = PubMaster(["modelV2", "drivingModelData", "cameraOdometry", "iqModelDataV2"])
  sm = SubMaster(["deviceState", "carState", "roadCameraState", "liveCalibration", "driverMonitoringState", "carControl", "liveDelay", "iqNavState", "radarState"])
  ...
  while True:
    ...
    if sm.updated["liveCalibration"] and sm.seen['roadCameraState'] and sm.seen['deviceState']:
      ...
      if device_from_calib_euler is None:
        ...
        if len(live_calib.rpyCalib) == 3:
          device_from_calib_euler = np.array(live_calib.rpyCalib, dtype=np.float32)
        else:
    ...
    long_action_t = long_delay + DT_MDL
    ...
    if "action_t" in model.numpy_inputs.keys():
    ...
    if model_output is not None:
      ...
      mdv2iq_send = messaging.new_message('iqModelDataV2')
      ...
      DH.update(sm['carState'], sm['carControl'].latActive, lane_change_prob, sm['iqNavState'], modelv2_send.modelV2, sm['radarState'])
      modelv2_send.modelV2.meta.laneChangeState = DH.lane_change_state
      modelv2_send.modelV2.meta.laneChangeDirection = DH.lane_change_direction
      mdv2iq_send.iqModelDataV2.laneTurnDirection = DH.lane_turn_direction
      ...
      pm.send('iqModelDataV2', mdv2iq_send)
