...
class MapdInstallManager:
  ...
  def download_needed(self) -> bool:
    ...
    if self.get_installed_version() != VERSION:
      return True
    # Re-download if the on-disk binary doesn't match the committed hash. Guards
    # the case where a git pull restores an older committed binary while the
    # MapdVersion param still reads VERSION (which would otherwise mask it).
  ...
  def _binary_hash_matches() -> bool:
    try:
      ...
      hash_path = os.path.join(BASEDIR, "iqpilot", "mapd", "tests", "mapd_hash")
      with open(hash_path) as f:
        expected = f.read().strip()
    except Exception:
      return True  # can't determine expected hash -> don't force a re-download
    if not expected:
      return True
    try:
      return get_file_hash(MAPD_PATH) == expected
    except Exception:
      return True
