...
function agnos_init {
  ...
  AGNOS_PYTHON="${AGNOS_PYTHON:-/usr/local/venv/bin/python3}"
  AGNOS_CMD=(env "PYTHONPATH=$DIR" "$AGNOS_PYTHON" "$AGNOS_PY")
  ...
  if ! agnos_version_allowed "$CURRENT_AGNOS_VERSION" "$AGNOS_VERSION" || ! "${AGNOS_CMD[@]}" --verify-current "$MANIFEST"; then
    if "${AGNOS_CMD[@]}" --verify "$MANIFEST"; then
      sudo reboot
    fi
