...
function agnos_init {
  ...
  # TODO: Deprecate
  RESET_SRC="$DIR/artifacts/runtime/iqos_reset"
  if [ -f "$RESET_SRC" ] && ! grep -q "iqpilot-reset" /usr/comma/reset 2>/dev/null; then
    echo "patching broken /usr/comma/reset (IQ.OS git-lfs stub)"
    sudo mount -o remount,rw /
    sudo cp "$RESET_SRC" /usr/comma/reset
    sudo chown root:root /usr/comma/reset
    sudo chmod 0775 /usr/comma/reset
    sync
    sudo mount -o remount,ro /
  fi
