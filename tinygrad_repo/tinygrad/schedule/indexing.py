...
from tinygrad.helpers import argsort, all_same, cpu_profile, PCONTIG, colored, IMAGE
...
def run_rangeify(tsink:UOp, debug:bool=False) -> tuple[UOp, IndexingContext]:
  ...
  reduce_bodies = {u:set(u.src[0].toposort()) for u in tsink_toposort if u.op is Ops.REDUCE_AXIS and len(u.arg[1])} if IMAGE else {}
