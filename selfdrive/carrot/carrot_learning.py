...
_LATERAL_MIN_CURVE = 10         # SteerActuatorDelay 추천을 위한 최소 커브 이벤트 수 (20 -> 10)
...
_STOP_GAP_WIDE_M = 6.0          # 정지 시 선행차와 이 거리 이상이면 '너무 멀리 정지'
_STOP_GAP_NEAR_M = 3.5          # 정지 시 선행차와 이 거리 이하이면 '너무 가까이 정지'
...
_PARAM_SPEC = {
  # 종방향 PID / 액추에이터 (문서2)
  "LongTuningKpV":      {"min": 0,    "max": 200,  "default": 100, "conv": 0.01,  "direction": 1,  "apply": "long"},
  ...
  "AutoCurveSpeedFactor": {"min": 80,  "max": 200,  "default": 120, "conv": 0.01,  "direction": 1,  "apply": "long"},
  # 정차 / 출발 (문서2)
  ...
  "StopDistanceCarrot": {"min": 400,  "max": 1000, "default": 550, "conv": 0.01,  "direction": 1,  "apply": "long"},
...
class CarrotLearner:
  ...
  def _calc_recommendations(self) -> dict:
    ...
    if apply_long and self._tfollow_speed_brake_acc >= _TFOLLOW_BRAKE_THRESHOLD_SEC:
