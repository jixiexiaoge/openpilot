...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    # 自动激活相关变量
    self.auto_activate_speed_threshold = 50.0  # 50km/h自动激活阈值
    self.auto_activate_enabled = True  # 启用自动激活功能
    self.auto_activate_triggered = False  # 标记是否已经触发过自动激活
    self.auto_activate_counter = 0  # 计数器，确保不会频繁触发
    self.system_ready = False  # 系统就绪标志
    self.system_init_time = time.time()  # 系统启动时间
  ...
  def update(self, can_parsers) -> structs.CarState: # type: ignore
    ...
    if self.CP.carFingerprint == CAR.QIYUAN_A05:
      ...
      if self.cruiseEnable and (self.iacc_enable_switch_button_rising_edge or ret.brakePressed):
          self.cruiseEnable = False
          self.auto_activate_triggered = False  # 重置自动激活标记
    ...
    # 系统就绪检查 - 确保系统启动后稳定运行一段时间
    current_time = time.time()
    if not self.system_ready and (current_time - self.system_init_time) > 5.0:  # 系统启动5秒后认为就绪
        self.system_ready = True

    # 简化的自动激活逻辑
    if (self.auto_activate_enabled and 
        self.system_ready and  # 系统已就绪
        not self.cruiseEnable and 
        not self.auto_activate_triggered and
        speed >= self.auto_activate_speed_threshold and
        not ret.brakePressed and
        not ret.gasPressed and
        cp_cam.vl["GW_31A"]["ACC_IACCHWAEnable"] == 1):  # 巡航系统可用
        
        # 激活巡航
        self.cruiseEnable = True
        self.auto_activate_triggered = True
        # 设置巡航速度为当前速度，取整到5的倍数
        if self.cruiseSpeed == 0:
            rounded_speed = round(speed / 5) * 5
            self.cruiseSpeed = rounded_speed if rounded_speed > 0 else 5

    # 当速度低于阈值时重置自动激活标记，以便下次可以再次触发
    if speed < (self.auto_activate_speed_threshold - 5):  # 设置5km/h的迟滞区间
        self.auto_activate_triggered = False
