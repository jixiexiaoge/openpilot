...
class IQLanePlanner:
  ...
  def get_d_path(self, v_ego: float, path_t: np.ndarray, path_xyz: np.ndarray,
                 adjust_lane_offset: float, input_time_offset: float, path_offset: float,
                 curve_speed: float = 0.0) -> tuple[np.ndarray, bool]:
    ...
    # curve offset: bias toward the inside of a turn (curve_speed sign = turn direction).
    # 0 when no turn-speed signal is available.
  ...
  def calculate_plan_yaw_and_yaw_rate(self, path_xyz: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
class IQLanePlannerController:
  ...
  def update(self, sm, measured_curvature: float) -> float | None:
    ...
    if self.lanelines_active:
      plan_yaw, plan_yaw_rate = self.lp.calculate_plan_yaw_and_yaw_rate(path_xyz)
    ...
    if mpc_nans or self.lat_mpc.solution_status != 0:
      ...
      if t > self.last_cloudlog_t + 5.0:
        ...
        cloudlog.warning("iq_lane_planner mpc invalid, fallback to model curvature")
      return None
