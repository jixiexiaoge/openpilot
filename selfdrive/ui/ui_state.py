...
from openpilot.selfdrive.ui.iqpilot.ui_state import IQUIState, IQDevice
  ...
  def _update_status(self) -> None:
    if self.started and self.sm.updated["selfdriveState"]:
      ...
      if computed_status != UIStatus.DISENGAGED:
        ...
        # Ignore transient IQ/selfdrive status races while the underlying engagement
        if self._last_non_disengaged_status != UIStatus.DISENGAGED:
          self.status = self._last_non_disengaged_status
        else:
          self.status = UIStatus.ENGAGED if ss.enabled else UIStatus.DISENGAGED
      elif (self._last_non_disengaged_status != UIStatus.DISENGAGED and
            (now - self._last_non_disengaged_time) < 2.0):
        self.status = self._last_non_disengaged_status
      else:
