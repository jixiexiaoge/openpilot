...
class SoftwareLayout(Widget):
  ...
  def _on_auth_branch(self):
    ...
    def on_username(result, username):
      ...
      IQInputDialog(tr("Git token / password"),
                    tr("leave username and token blank to clear"),
                    password_mode=True, callback=on_token).show()

    IQInputDialog(tr("Git username"), tr("for private branch updates"),
                  current_text=current_user, callback=on_username).show()
