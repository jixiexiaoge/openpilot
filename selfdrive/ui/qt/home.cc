...
// AutoTunerDialog
AutoTunerDialog::AutoTunerDialog(const QString &title_text, const QJsonObject &recs, QWidget *parent) : DialogBase(parent), recommendations(recs) {
  setWindowFlags(Qt::Popup | Qt::FramelessWindowHint);
  setAttribute(Qt::WA_TranslucentBackground);
  setStyleSheet(R"(
    DialogBase { background: transparent; }
  ...
  QVBoxLayout *outer_layout = new QVBoxLayout(this);
  outer_layout->setContentsMargins(200, 40, 200, 40);
  ...
  connect(btn_guide, &QPushButton::clicked, this, [=]() {
    ...
    if (QString::fromStdString(Params().get("LanguageSetting")) != "main_ko") {
      ...
      - <b>TFollowGap1~4</b>: GAP 단계별 추종 거리 시간(x0.01초). 낮을수록 가까워집니다. 최소 0.70초 보장.<br>
...
void HomeWindow::updateState(const UIState &s) {
  ...
  if (carrot_tuner_frame++ % 20 == 0) {
    ...
    if (params.getBool("CarrotLearningPopupReady")) {
      // 중복 팝업/자동 적용 처리를 방지하기 위해 즉시 플래그 해제
