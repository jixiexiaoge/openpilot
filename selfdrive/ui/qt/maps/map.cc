...
void MapWindow::initLayers() {
  ...
  if (!m_map->layerExists("buildingsLayer")) {
    ...
    if (!m_map->sourceExists("carrotSpeedSource")) {
      ...
      // 锟斤拷 FeatureCollection GeoJSON (QVariantMap 锟斤拷锟斤拷)
      QVariantMap fc;
      fc["type"] = "FeatureCollection";
      fc["features"] = QVariantList{};   // 锟斤拷 锟斤拷锟斤拷飘
    ...
    if (!m_map->layerExists("carrotSpeedLayer")) {
      ...
      // properties.speed 锟斤拷 锟截斤拷飘锟斤拷 钎锟斤拷 (锟斤拷奴 锟斤拷锟斤拷)
      // "{speed}" 锟斤拷锟斤拷锟斤拷 properties.speed 锟斤拷锟斤拷 锟斤拷锟节匡拷锟斤拷 锟街撅拷锟斤拷
...
void MapWindow::updateState(const UIState &s) {
  ...
  if (sm.updated("modelV2")) {
    // set path color on change, and show map on rising edge of navigate on openpilot
      ...
      bool nav_enabled = sm["carrotMan"].getCarrotMan().getActiveCarrot() > 1; // sm["modelV2"].getModelV2().getNavEnabled();
  ...
  if (sm.updated("navRoute") && sm["navRoute"].getNavRoute().getCoordinates().size()) {
    ...
    if (allow_open) {
      ...
      uiState()->scene._display_time_count = 100;
    }
  }
    ...
    if (!raw.empty()) {
      ...
      if (qraw != last_viz_raw) {
        ...
        if (err.error == QJsonParseError::NoError && doc.isObject()) {
          ...
          // GeoJSON FeatureCollection 锟斤拷 QVariantMap 飘锟斤拷锟斤拷 锟斤拷锟斤拷锟�
          QVariantList features;  // Feature 锟斤拷锟斤拷飘
          ...
          for (const QJsonValue& v : pts) {
            ...
            // geometry: Point (GeoJSON: [lon, lat] 锟斤拷锟斤拷)
            QVariantList coords;
            coords.append(plon);
            coords.append(plat);
  ...
  if (sm.rcv_frame("navRoute") != route_rcv_frame || allow_open) {
    clearRoute();
