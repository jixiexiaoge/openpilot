...
from openpilot.selfdrive.ui.iqpilot.onroad.model_renderer import ChevronMetrics, IQModelRenderer
...
# Path coloring when Lane Planner 2 is actively steering from lane lines
LANELINE_ACTIVE_COLORS = [
  rl.Color(68, 150, 255, 115),
  rl.Color(40, 120, 255, 92),
  rl.Color(40, 120, 255, 0),
]
...
class ModelRenderer(Widget, ChevronMetrics, IQModelRenderer):
  def __init__(self):
    ...
    # Lane Planner 2 status (throttled param read for path coloring)
    self._iqlp_status = "OFF"
    self._iqlp_status_refresh = 0
  ...
  def _draw_path(self, sm):
    ...
    self._iqlp_status_refresh -= 1
    if self._iqlp_status_refresh <= 0:
      self._iqlp_status_refresh = 20
      self._iqlp_status = ui_state.params.get("IQLanePlannerStatus", encoding='utf-8') or "OFF"

    if self._iqlp_status == "LANELINE":
      gradient = Gradient(
        start=(0.0, 1.0),
        end=(0.0, 0.0),
        colors=LANELINE_ACTIVE_COLORS,
        stops=[0.0, 0.5, 1.0],
      )
