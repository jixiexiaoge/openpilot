...
class AugmentedRoadView(CameraView, AugmentedRoadViewIQ):
  ...
  def _draw_border(self, rect: rl.Rectangle):
    ...
    rl.draw_rectangle_rounded_lines_ex(border_rect, border_roundness, 10, UI_BORDER_SIZE, rl.BLACK)

    border_color = BORDER_COLORS.get(ui_state.status, BORDER_COLORS[UIStatus.DISENGAGED])
    if ui_state.status != UIStatus.DISENGAGED and not IS_MICI:
      bottom_bar_margin = UI_BORDER_SIZE * 4
      rl.draw_rectangle(
        int(border_rect.x + bottom_bar_margin),
        int(border_rect.y + border_rect.height - UI_BORDER_SIZE / 2),
        int(border_rect.width - 2 * bottom_bar_margin),
        UI_BORDER_SIZE,
        border_color,
      )
