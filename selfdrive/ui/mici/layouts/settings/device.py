...
from openpilot.iqpilot.konn3kt.registration import ensure_dev_pairing_identity
...
def _engaged_confirmation_callback(callback: Callable, action_text: str):
  if not ui_state.engaged:
    ...
    if action_text == "power off":
      ...
      # TODO: check
      icon = "icons_mici/settings/konn3kt_icon.png"
...
class DeviceInfoLayoutMici(Widget):
  def __init__(self):
    ...
    self._dongle_id_text_label = MiciLabel(get_cached_dongle_id(params, prefer_readonly=True) or 'N/A', 32, width=max_width, color=subheader_color, font_weight=FontWeight.ROMAN)
...
class PairBigButton(NeonBigButton):
  KONN3KT_ONLINE_NS = 80_000_000_000  # 80 seconds in nanoseconds
  _ICON_SIZE = 48

  def __init__(self):
    super().__init__("konn3kt", chips=["pair in app"])
    self._konn3kt_icon = gui_app.texture("icons_mici/settings/konn3kt_icon.png", self._ICON_SIZE, self._ICON_SIZE)
  ...
  def _update_state(self):
    if ui_state.prime_state.is_paired():
      status = "online" if self._is_konn3kt_online() else "offline"
      self._title_text = "konn3kt"
      self._label._text = "konn3kt"
      self.set_chips([status])
    else:
      self._title_text = "konn3kt"
      self._label._text = "konn3kt"
      self.set_chips(["pair in app"])

  def _render(self, _):
    super()._render(_)

    icon_x = int(self._rect.x + self._rect.width - self._ICON_SIZE - 12)
    icon_y = int(self._rect.y + 10)
    rl.draw_texture_ex(self._konn3kt_icon, rl.Vector2(icon_x, icon_y), 0.0, 1.0, rl.WHITE)

    if ui_state.prime_state.is_paired():
      is_online = self._is_konn3kt_online()
      status_color = rl.Color(134, 255, 78, 255) if is_online else rl.Color(201, 34, 49, 255)
      dot_x = int(self._rect.x + 20)
      dot_y = int(self._rect.y + 20)
      rl.draw_circle(dot_x, dot_y, 10, status_color)
  ...
  def _handle_mouse_release(self, mouse_pos: MousePos):
    ...
    if not system_time_valid():
      ...
      if PC and os.getenv("KONN3KT_DEV_PAIRING") == "1":
        try:
          ensure_dev_pairing_identity(Params(), force_reset=os.getenv("KONN3KT_DEV_PAIRING_RESET") == "1")
        except Exception:
          pass

      if BaseApi.get_key_pair()[0] is None:
        dlg = BigDialog(tr("No device keys found"), "")
      else:
        dlg = PairingDialog()
    gui_app.set_modal_overlay(dlg)
...
class UpdateOpenpilotBigButton(NeonBigButton):
  def __init__(self):
    super().__init__("update IQ.Pilot")
    self._status_value = ""
  ...
  def set_value(self, value: str):
    ...
    self.set_chips([value] if value else [])
...
class DeviceLayoutMici(NavWidget):
  def __init__(self, back_callback: Callable):
    ...
    regulatory_btn = NeonBigButton("regulatory info")
    regulatory_btn.set_click_callback(self._on_regulatory)

    driver_cam_btn = NeonBigButton("driver camera preview")
    driver_cam_btn.set_click_callback(self._show_driver_camera)
    driver_cam_btn.set_enabled(lambda: ui_state.is_offroad())

    review_training_guide_btn = NeonBigButton("review training guide")
    review_training_guide_btn.set_click_callback(self._on_review_training_guide)
    review_training_guide_btn.set_enabled(lambda: ui_state.is_offroad())

    self._big_ui_toggle = NeonBigParamToggle("force mici UI", "ForceSmallUI")
    self._big_ui_toggle.set_visible(HARDWARE.get_device_type() in ('tici', 'tizi'))
