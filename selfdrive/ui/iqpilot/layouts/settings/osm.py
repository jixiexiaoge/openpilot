...
from openpilot.system.ui.iqpilot.widgets.tree_dialog import TreeFolder, TreeNode, TreeOptionDialog
...
class OSMLayout(Widget):
  ...
  def _initialize_items(self):
    ...
    self._delete_maps_btn = IQListItem(tr("Downloaded Maps"), action_item=NoElideButtonAction(tr("DELETE"), enabled=True), callback=self._delete_maps)
    self._progress = progress_item(tr("Downloading Map"))
    self._update_btn = IQListItem(tr("Database Update"), action_item=NoElideButtonAction(tr("CHECK"), enabled=True), callback=self._update_db)
    self._country_btn = IQListItem(tr("Country"), action_item=NoElideButtonAction(tr("SELECT"), enabled=True), callback=lambda: self._select_region("Country"))
    self._state_btn = IQListItem(tr("State"), action_item=NoElideButtonAction(tr("SELECT"), enabled=True), callback=lambda: self._select_region("State"))
  ...
  def _trigger_download(self):
    ...
    state = ui_state.params.get("OsmStateName") or ""
    nations = [country] if country else []
  ...
  def _update_labels(self):
    ...
    if downloading or pending:
      ...
      self._update_btn.action_item.set_enabled(not downloading)  # TODO-IQ: introduce CANCEL database download with mapd
