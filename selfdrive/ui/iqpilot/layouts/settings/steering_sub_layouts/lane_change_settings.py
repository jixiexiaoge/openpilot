...
class LaneChangeSettingsLayout(Widget):
  ...
  def _initialize_items(self):
    ...
    self._continuous = toggle_item(
      ...
      description=lambda: tr("By default a lane change ends after one maneuver, so the system returns to normal lane " +
                             "keeping even if the blinker stays on. Toggle to allow chaining additional lane changes " +
                             "while the blinker is held (a nudge on the steering wheel is still required for each " +
                             "follow-up change)."),
    )
