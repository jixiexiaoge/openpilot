...
class NavMapPanel(Widget):
  ...
  def _draw_route_overlay(self, map_rect: rl.Rectangle):
    if len(self._projected_route_points) >= 2:
      ...
      margin = 18.0  # widest line stroke, so a segment grazing the edge still draws
      points = self._projected_route_points
      for idx in range(len(points) - 1):
        x1, y1 = points[idx]
        x2, y2 = points[idx + 1]
        if ((x1 < -margin and x2 < -margin) or (x1 > width + margin and x2 > width + margin) or
            (y1 < -margin and y2 < -margin) or (y1 > height + margin and y2 > height + margin)):
          continue
