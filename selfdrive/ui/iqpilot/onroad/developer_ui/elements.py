...
class LateralControlElement:
  ...
  def get_lat_color(lat_active: bool, steer_override: bool, angle_steers: float = 0.0,
    ...
    color = rl.WHITE
    if lat_active:
      color = rl.Color(145, 155, 149, 255) if steer_override else ACTIVE_ACCENT
...
class SteeringAngleElement(LateralControlElement):
  ...
  def update(self, sm, is_metric: bool) -> UiElement:
    ...
    angle_offset = sm['liveParameters'].angleOffsetAverageDeg if sm.valid['liveParameters'] else 0.0
    angle_steers_corrected = angle_steers - angle_offset
    lat_active = sm['carControl'].latActive
    steer_override = car_state.steeringPressed

    value = f"{angle_steers_corrected:.1f}°"
    color = self.get_lat_color(lat_active, steer_override, angle_steers_corrected, check_angle=True)

    return UiElement(value, "R.S.", self.unit, color)
...
class DesiredSteeringAngleElement(LateralControlElement):
  ...
  def update(self, sm, is_metric: bool) -> UiElement:
    ...
    lat_active = sm['carControl'].latActive
    angle_offset = sm['liveParameters'].angleOffsetAverageDeg if sm.valid['liveParameters'] else 0.0
    angle_steers = car_state.steeringAngleDeg - angle_offset
    lateral_mode = controls_state.lateralControlState.which()
    if lateral_mode == 'angleState':
      steer_angle_desired = controls_state.lateralControlState.angleState.steeringAngleDesiredDeg - angle_offset
    else:
      # Joystick/debug mode publishes the commanded angle on carControl actuators.
      steer_angle_desired = car_control.actuators.steeringAngleDeg - angle_offset
    ...
    return UiElement(value, "D.S.", self.unit, color)
...
class ActualLateralAccelElement(LateralControlElement):
  ...
  def update(self, sm, is_metric: bool) -> UiElement:
    ...
    return UiElement(value, "A.L.A.", self.unit, color)
...
class DesiredLateralAccelElement(LateralControlElement):
  ...
  def update(self, sm, is_metric: bool) -> UiElement:
    ...
    return UiElement(value, "D.L.A.", self.unit, color)
...
class DesiredSteeringPIDElement(LateralControlElement):
  ...
  def update(self, sm, is_metric: bool) -> UiElement:
    ...
    angle_offset = sm['liveParameters'].angleOffsetAverageDeg if sm.valid['liveParameters'] else 0.0
    angle_steers = car_state.steeringAngleDeg - angle_offset
    steer_angle_desired = controls_state.lateralControlState.pidState.steeringAngleDesiredDeg - angle_offset
...
class AEgoElement:
  ...
  def update(self, sm, is_metric: bool) -> UiElement:
    ...
    value = f"{a_ego:.1f}"
    return UiElement(value, "L.ACC.", self.unit, rl.WHITE)
