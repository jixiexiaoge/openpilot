...
from openpilot.selfdrive.frogpilot.frogpilot_modeld.runners.runmodel_pyx import RunModel, Runtime
assert Runtime
...
class ModelRunner(RunModel):
  ...
  def __new__(cls, paths, *args, **kwargs):
    if ModelRunner.THNEED in paths and USE_THNEED:
      from openpilot.selfdrive.frogpilot.frogpilot_modeld.runners.thneedmodel_pyx import ThneedModel as Runner
      runner_type = ModelRunner.THNEED
    elif ModelRunner.SNPE in paths and USE_SNPE:
      from openpilot.selfdrive.frogpilot.frogpilot_modeld.runners.snpemodel_pyx import SNPEModel as Runner
      runner_type = ModelRunner.SNPE
    elif ModelRunner.ONNX in paths:
      from openpilot.selfdrive.frogpilot.frogpilot_modeld.runners.onnxmodel import ONNXModel as Runner
