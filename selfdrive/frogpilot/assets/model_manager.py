...
class ModelManager:
  ...
  def copy_default_model():
    classic_default_model_path = Path(MODELS_PATH) / f"{DEFAULT_CLASSIC_MODEL}.thneed"
    ...
    default_model_path = Path(MODELS_PATH) / f"{DEFAULT_MODEL}.thneed"
  ...
  def handle_verification_failure(self, model, model_path):
    ...
    download_file(self.cancel_download_param, model_path, self.download_progress_param, model_url, self.download_param, params_memory)
  ...
  def download_model(self, model_to_download):
    model_path = Path(MODELS_PATH) / f"{model_to_download}.thneed"
    ...
    if not repo_url:
      handle_error(model_path, "GitHub and GitLab are offline...", "Repository unavailable", self.download_param, self.download_progress_param, params_memory)
      return
  ...
  def check_models(self, available_models, repo_url):
    ...
    for model in available_models:
      model_path = Path(MODELS_PATH) / f"{model}.thneed"
  ...
  def validate_models(self):
    ...
    current_model_path = Path(MODELS_PATH) / f"{current_model}.thneed"
    ...
    for model_file in Path(MODELS_PATH).iterdir():
      if model_file.is_file() and model_file.suffix == ".thneed":
        ...
        if model_name not in available_models.split(','):
          ...
          print(f"Deleted model file: {model_file.name} - Reason: Model is not in the list of available models")
