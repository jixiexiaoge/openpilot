...
class LiveStreamVideoStreamTrack(TiciVideoStreamTrack):
  ...
  async def recv(self):
    ...
    if not self._sent_keyframe:
      ...
      # Got the IDR we asked for — stop nagging the encoder.
    ...
    # Pace output to the real-time camera cadence. aiortc pulls recv() as fast as it returns, so
    # without pacing the (deprioritized) daemon drains a socket backlog in a burst — the client
    # gets a flood of frames then a stall (the 1->20 fps swings). next_pts() sleeps until each
    # frame's scheduled emit time and returns an evenly spaced PTS for a steady ~20fps stream.
