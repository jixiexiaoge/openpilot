...
IGNORE_PARTITIONS = {"splash"}
IGNORE_CURRENT_PARTITIONS = IGNORE_PARTITIONS | {"system"}
...
def get_current_slot_number() -> int:
  return 1 - get_target_slot_number()
...
def flash_agnos_update(manifest_path: str, target_slot_number: int, cloudlog, standalone=False) -> None:
  ...
  for partition in update:
    if partition["name"] in IGNORE_PARTITIONS:
      cloudlog.info(f"Skipping {partition['name']}")
      continue
...
def verify_agnos_update(manifest_path: str, target_slot_number: int, ignore_partitions: set[str] | None = None) -> bool:
  update = json.load(open(manifest_path))
  ignore_partitions = ignore_partitions or IGNORE_PARTITIONS
  return all(verify_partition(target_slot_number, partition) for partition in update if partition["name"] not in ignore_partitions)


def verify_current_agnos_update(manifest_path: str) -> bool:
  return verify_agnos_update(manifest_path, get_current_slot_number(), IGNORE_CURRENT_PARTITIONS)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--verify-current", action="store_true", help="Verify the currently booted AGNOS slot")
  ...
  if args.verify_current:
    exit(0 if verify_current_agnos_update(args.manifest) else 1)
  elif args.verify:
