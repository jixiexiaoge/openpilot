...
class DaemonProcess(ManagerProcess):
  ...
  @staticmethod
  def should_run(started, params, CP, classic_model, frogpilot_model, frogpilot_toggles):
    return True
...
def ensure_running(procs: ValuesView[ManagerProcess], started: bool, params=None, CP: car.CarParams=None,
                   not_run: list[str] | None=None, classic_model=False, frogpilot_model=False, frogpilot_toggles=None) -> list[ManagerProcess]:
  if not_run is None:
    not_run = []

  running = []
  for p in procs:
    if p.enabled and p.name not in not_run and p.should_run(started, params, CP, classic_model, frogpilot_model, frogpilot_toggles):
      running.append(p)
    else:
      p.stop(block=False)
