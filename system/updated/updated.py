...
def configure_git_auth(cwd: str) -> None:
  ...
  try:
    from openpilot.iqpilot.konn3kt.iqlvbs.git_remote import configure
    configure(cwd)
  except Exception:
...
def handle_agnos_update() -> None:
  from openpilot.system.hardware.tici.agnos import flash_agnos_update, get_target_slot_number, verify_current_agnos_update

  cur_version = HARDWARE.get_os_version()
  device_type = HARDWARE.get_device_type()
  is_tici_c3 = device_type in ("tici", "three")
  updated_version, compat_versions = get_agnos_target_versions(OVERLAY_MERGED)
  ...
  version_allowed = agnos_version_allowed(cur_version, updated_version, compat_versions)
  active_matches_manifest = verify_current_agnos_update(manifest_path)
  cloudlog.info(
    f"AGNOS version check: current={cur_version}, target={updated_version}, compat={compat_versions}, active_matches_manifest={active_matches_manifest}"
  )
  if version_allowed and active_matches_manifest:
    return
...
class Updater:
  ...
  def set_params(self, update_success: bool, failed_count: int, exception: str | None) -> None:
    ...
    def get_description(basedir: str) -> str:
      ...
      try:
        ...
        with open(os.path.join(basedir, "iqpilot", "common", "version.h")) as f:
  ...
  def check_for_update(self) -> None:
    ...
    configure_git_auth(OVERLAY_MERGED)
